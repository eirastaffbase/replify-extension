...
(function() {
    ...
    const normalisePercentage = (inputValue, cap = MAX_DESIRED_PCT) => {
        ...
        if (n >= 90) { // Very high input (90-100)
            newPct = rand( Math.min(cap - 30, 50), Math.min(cap - 5, 65) ); // e.g., target 50-65 (if cap allows)
        } else if (n >= 75) { // High input (75-89)
            newPct = rand( Math.min(cap - 35, 45), Math.min(cap - 10, 60) ); // e.g., target 45-60
        } else if (n >= 50) { // Medium input (50-74)
            newPct = rand( Math.min(cap - 40, 35), Math.min(cap - 15, 55) ); // e.g., target 35-55
        } else if (n >= 25) { // Low-Medium input (25-49)
            newPct = rand( Math.min(cap - 45, MIN_PCT_FLOOR_INPUTS + 5), Math.min(cap - 20, 45) ); // e.g., target 10-45
        } else { // Very low input (0-24)
            newPct = rand( MIN_PCT_FLOOR_INPUTS, Math.min(cap - 50, Math.max(MIN_PCT_FLOOR_INPUTS, n + rand(0, 10))) ); // Keep it low, but give a floor
        }
    ...
    const harmonisePercentagePair = (visPctInput, engPctInput, cap = MAX_DESIRED_PCT) => {
        ...
        if (visPct >= cap) visPct = cap - rand(1, Math.min(3, cap > 1 ? cap-1 : 1)); // Nudge down from cap
        if (engPct >= cap) engPct = cap - rand(1, Math.min(3, cap > 1 ? cap-1 : 1)); // Nudge down from cap
    ...
    const generateAlignmentScore = () => { /* ... जस का तस ... */
    ...
    function generateAlignmentAnswers(avgScore, numParticipants) { /* ... जस का तस ... */
    ...
    function extractCampaignIdFromUrl(url, paramName = 'campaignId') { /* ... जस का तस ... */
        try {
    ...
    const injectedCustomFetch = async function(...args) {
        ...
        if (typeof url === 'string') {
             if (url.startsWith(window.location.origin) || url.startsWith("https://app.staffbase.com") || url.includes("/api/")) {
                ...
                switch (matchedEndpointType) {
                    case 'RANKINGS':
                        if (data.ranking && Array.isArray(data.ranking)) {
                            data.ranking.forEach(item => {
                                ...
                                if (potential > 0) { item.visitors = Math.min(item.visitors, potential); }
                    ...
                    case 'CAMPAIGN_STATS':
                        if (campaignId && data) {
                            ...
                            if (injectedCampaignDataStore[campaignId] && injectedCampaignDataStore[campaignId].visitors !== undefined) { // Check if already processed by RANKINGS
                                ...
                                if (potential > 0) { data.visitors = Math.min(data.visitors, potential); }
                                ...
                                injectedCampaignDataStore[campaignId] = { 
                                    ...injectedCampaignDataStore[campaignId], 
                                    visitors: data.visitors, engagedUsers: data.engagedUsers, potentialVisitors: potential, 
                                    normVisPercent, normEngPercent 
                                };
                            }
                        }
                        break;
                    case 'POST_STATS':
                        if (campaignId && data && Array.isArray(data.posts)) {
                            ...
                            data.posts.forEach(post => {
                                let postPotential = Math.max(1, /* ... your existing postPotential logic ... */
                                    Math.min(post.visitors, campData.potentialVisitors > 0 ? Math.ceil(campData.potentialVisitors / (data.posts.length || 1) * randFloat(0.5, 1.5)) : rand(10,50) )
                                );
                                ...
                                let postVisPercent = rand(MIN_PCT_FLOOR_FOR_HIGH_INPUTS, MAX_DESIRED_PCT - rand(5,15) ); // e.g., 25 to 73-83
                                let postEngPercent = rand(MIN_PCT_FLOOR_FOR_HIGH_INPUTS - 5, Math.max(MIN_PCT_FLOOR_FOR_HIGH_INPUTS - 5, postVisPercent - rand(5,10)));
                                ...
                                if (typeof campData.alignmentScore === 'number' && campData.alignmentScore > 0) { if (Math.random() < 0.65) postGetsSurveyActivity = true; }
                                ...
                                if (!injectedCampaignDataStore[campaignId]) injectedCampaignDataStore[campaignId] = { postSurveyAnswers: {} }; // Ensure object exists
                    ...
                    case 'ENGAGEMENT_GROUPS': 
                        if (data.ranking && Array.isArray(data.ranking)) {
                            data.ranking.forEach(group => {
                                ...
                                if (group.visitors > 0) { // Use group.visitors as the base for engagers for this endpoint
                    ...
                    case 'VISIBILITY_USER_GROUP_RANKING': // ... 
                        if (data.ranking && Array.isArray(data.ranking)) {
                            data.ranking.forEach(group => {
                                const potential = group.potentialVisitors > 0 ? group.potentialVisitors : group.visitors; // Use potential if available
                                if (potential > 0) {
                                    let visPct = normalisePercentage(rand(MIN_PCT_FLOOR_FOR_HIGH_INPUTS, MAX_DESIRED_PCT), MAX_DESIRED_PCT); // Normalize a random %
                                ...
                                if (group.potentialVisitors === 0 && group.visitors > 0) { // If potential was 0 but we generated visitors
                    ...
                    case 'VISIBILITY_TIMESERIES': // ...  uses injectedCampaignDataStore ...
                        if (data.timeseries && Array.isArray(data.timeseries)) {
                            ...
                            data.timeseries.forEach(ts => {
                                const dayMaxPotential = rand(Math.floor(overallMaxSeen * 0.05), Math.floor(overallMaxSeen * 0.3)); // Smaller daily fraction
